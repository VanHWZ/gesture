...
class Ui_MainWindow(QtWidgets.QMainWindow):
    ...
    def openfile(self):
        ...
        if name != "":
            ...
            index = int(np.argwhere(frq1 == 21000.745738636364))
            print(21000.745738636364, abs(Y1)[index])

            plt.xlabel('Freq (Hz)')
            plt.ylabel('|Y(freq)|')
            plt.xlim(left=20750)
            plt.xlim(right=21250)

            plt.subplot(223)
            plt.specgram(waveData[0], Fs=framerate, NFFT=1024, noverlap=512, scale_by_freq=False, sides='default')
            plt.ylabel('Frequency(Hz)')
            plt.xlabel('Time(s)')
            plt.ylim(bottom=20750)
            plt.ylim(top=21250)
